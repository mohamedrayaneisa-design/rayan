...
import { Search, ChevronDown, Database, Check, FileText } from 'lucide-react';
import { soundService } from '../services/soundService';

interface MsanSelectorProps {
  options: string[];
  value: string;
...
export const MsanSelector: React.FC<MsanSelectorProps> = ({ options, value, onChange, disabled, className = "", compact = false, activeTab }) => {
      ...
      >
         {value ? (
             <div className="flex items-center gap-3 w-full overflow-hidden">
                <div className={`flex items-center justify-center rounded-lg bg-cyan-950/30 border border-cyan-500/20 shrink-0 ${compact ? 'w-5 h-5' : 'w-8 h-8'}`}>
                    {activeTab === 'recap' ? (
                        ...
                        <Database className={`${compact ? 'w-3 h-3' : 'w-4 h-4'} text-cyan-400`} />
                    )}
                </div>
                 <div className="flex flex-col items-start overflow-hidden gap-0.5">
                    <span className="text-[7px] font-black text-slate-500 uppercase tracking-widest leading-none">
                        {activeTab === 'recap' ? 'CMD NETO SÉLECTIONNÉ' : 'MSAN SÉLECTIONNÉ'}
                    </span>
             ...
             <div className="flex items-center gap-3 w-full pl-1">
                {activeTab === 'recap' ? (
                    <FileText className={`${compact ? 'w-3 h-3' : 'w-4 h-4'} text-slate-600 group-hover:text-cyan-500 transition-colors shrink-0`} />
                ) : (
                    <Database className={`${compact ? 'w-3 h-3' : 'w-4 h-4'} text-slate-600 group-hover:text-cyan-500 transition-colors shrink-0`} />
                )}
                <span className={`${compact ? 'text-[10px]' : 'text-xs'} font-bold text-slate-500 uppercase tracking-widest group-hover:text-slate-300 transition-colors truncate`}>
                    {activeTab === 'recap' ? 'CMD NETO' : 'TOUS LES MSAN...'}
                </span>
             </div>
         )}
      ...
      {isOpen && (
        <div className="absolute top-full left-0 right-0 mt-2 bg-slate-950/95 backdrop-blur-xl border border-cyan-500/30 rounded-xl shadow-[0_10px_40px_rgba(0,0,0,0.5)] z-[100] overflow-hidden flex flex-col max-h-[300px] min-w-[200px] animate-fade-in-up origin-top">
           ...
           <div className="overflow-y-auto custom-scrollbar p-1.5 space-y-0.5">
              <div 
                 className="px-3 py-2.5 hover:bg-white/5 rounded-lg cursor-pointer text-[10px] font-bold text-slate-500 hover:text-white transition-all uppercase tracking-wider flex items-center gap-2 group/opt"
                 onClick={() => { 
                   soundService.playClick();
                   onChange(""); 
                   setIsOpen(false); 
              ...
              >
                 <div className={`w-1.5 h-1.5 rounded-full ${!value ? 'bg-cyan-500 shadow-[0_0_8px_rgba(34,211,238,0.5)]' : 'bg-slate-700 group-hover/opt:bg-slate-500'}`}></div>
                 {activeTab === 'recap' ? 'CMD NETO' : 'TOUS LES MSAN...'}
              </div>
              ...
              {filteredOptions.length > 0 ? (
                  filteredOptions.map(opt => (
                    <div 
                        key={opt}
                        className={`px-3 py-2.5 hover:bg-cyan-950/30 rounded-lg cursor-pointer text-[10px] font-bold transition-all uppercase tracking-wider flex items-center justify-between group/opt ${value === opt ? 'bg-cyan-950/50 text-cyan-400 border border-cyan-500/10' : 'text-slate-300 border border-transparent'}`}
                        onClick={() => { 
                          soundService.playClick();
                          onChange(opt); 
                          setIsOpen(false); 
                    ...
                    >
                        ...
                        {value === opt && <Check className="w-3.5 h-3.5 text-cyan-500 animate-fade-in-up" />}
                    </div>
                  ))
              ) : (
                  <div className="px-3 py-8 text-center flex flex-col items-center justify-center opacity-50">
                     ...
                     <span className="text-[10px] text-slate-500 font-bold uppercase tracking-wider">
                         {activeTab === 'recap' ? 'Aucun CMD NETO trouvé' : 'Aucun MSAN trouvé'}
                     </span>
                  </div>
              )}
           </div>
